...
#include "CaptainsCommon.as"  // Waffle: Prevent menu if player can't swap teams
...
{
	// Waffle: Check if we can swap teams
    CPlayer@ player = getLocalPlayer();
	if (player is null)
	{
		return;
    ...
    {
        client_AddToChat("Swapping teams is disabled!", ConsoleColour::ERROR);
        Sound::Play("NoAmmo.ogg");
        return;
    }
