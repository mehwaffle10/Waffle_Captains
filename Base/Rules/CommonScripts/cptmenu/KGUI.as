...
// VAM EDIT: ANY EDITS STARTS WITH `Vam:`
...
class GenericGUIItem : IGUIItem{
	...
	private void invokeClickListeners(int x, int y, int buttonCode){
		...
		if (_clickListeners.length > 0){
			...
			{
			   if (getLocalPlayer() !is null && getLocalPlayer().getBlob() !is null)
			      getLocalPlayer().getBlob().set_bool("GUIEvent",false);
			}
		}
	}
  ...
  // Vam: Cleaned up
		...
		for(int i = 0; i < children.length; i++){
		 IGUIItem@ item = children[i];
			if(!item.isEnabled) continue;
			item.position = position+item.localPosition;
			item.draw();
		}
		updateToolTipState();
	...
	private void updateMouseStates(){
		...
		_mousePosition = controls.getInterpMouseScreenPos();
	...
	void updateDraggingStates(){
		...
		if(_isDragging && !_mouseLeftButtonHold){
			_isDragging = false;
			_isDragPossible = true;
			// Vam: Required for childs
			localPosition = localPosition + (_dragCurrentPosition - _dragStartPosition);
...
class Window : GenericGUIItem{
	...
	void drawSelf(){
	 // Vam: changed DrawWindow to DrawFramedPane
		GUI::DrawFramedPane(position, position+size);
...
class Label : GenericGUIItem{
	...
	void drawSelf(){
	 // Vam: Changed drawRulesFont to DrawText for better flexability
		GUI::DrawText(label,position,position+size,color,centered,false, false);
		GenericGUIItem::drawSelf();
	}
